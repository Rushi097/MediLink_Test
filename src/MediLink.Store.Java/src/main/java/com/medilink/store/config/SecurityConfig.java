...
public class SecurityConfig {
    ...
    @Bean SecurityFilterChain securityFilterChain(HttpSecurity http, StoreUserDetailsService users) throws Exception {
        return http.userDetailsService(users).authorizeHttpRequests(auth -> auth
                .requestMatchers("/css/**", "/uploads/**", "/login", "/register", "/error").permitAll().anyRequest().hasRole("STORE_OWNER"))
