...
public class StoreInventoryService {
    ...
    public void updateProduct(Store store, UUID medicineId, ProductForm form, String imageUrl) {
        ...
        if (imageUrl == null) {
            jdbc.update(
                    "UPDATE Medicines SET Name = ?, Category = ?, Description = ?, Price = ?, StockQuantity = ? WHERE Id = ?",
                    form.getName().trim(), form.getCategory().trim(), form.getDescription().trim(), form.getPrice(),
                    form.getStockQuantity(), medicineId.toString());
        } else {
            jdbc.update(
                    "UPDATE Medicines SET Name = ?, Category = ?, Description = ?, Price = ?, StockQuantity = ?, ImageUrl = ? WHERE Id = ?",
                    form.getName().trim(), form.getCategory().trim(), form.getDescription().trim(), form.getPrice(),
                    form.getStockQuantity(), imageUrl, medicineId.toString());
        }
    }
}
